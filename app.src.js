...
(() => {
  ...
  const POP = ["#d94f92","#2bb2b6","#cfae3e","#d9662f","#8a68d6","#4d82db","#46b862"];
  let W, H, drips = [];
  function newDrip(){
  ...
  function draw(){
    ...
    const bw=W/2, bh=H/2, quad=["#d94f92","#2bb2b6","#cfae3e","#8a68d6"];
    ...
    ctx.globalAlpha=1;
    // spray-paint drips creeping down from the top edge (grimy neon graffiti)
    ctx.globalAlpha=0.28;
    drips.forEach((d,i)=>{
      if(d.len<d.max) d.len+=d.v;
      ctx.shadowColor=d.col; ctx.shadowBlur=7;
